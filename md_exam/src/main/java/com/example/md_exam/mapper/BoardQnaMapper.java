...
public interface BoardQnaMapper {
    @Insert("INSERT INTO qna VALUES(null, #{subject}, #{writer}, #{content}, 0, now(), #{grp}, #{seq}, 1, #{isFiles}, 0, #{uIdFk})")
    ...
    @Delete("DELETE FROM qna WHERE grp = #{grp}")
    void setReplyDelete(int grp);
}
