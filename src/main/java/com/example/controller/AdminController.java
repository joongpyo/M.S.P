...
import com.example.mapper.AdminBoardMapper;
...
public class AdminController {
    ...
    @ResponseBody
    public Map<String, Object> setDisUpdate(@ModelAttribute com.example.dto.DiseaseDto diseaseDto, @RequestParam String disName ){
    ...
    public String disDelete(@ModelAttribute com.example.dto.DiseaseDto diseaseDto){
    ...
    @ResponseBody
    public Map<String,Object> setNoticeInsert(@ModelAttribute com.example.dto.AdminBoardDto adminBoardDto, @RequestParam("file") MultipartFile file) throws IOException {
        ...
        if(adminBoardDto.getConfigCode().equals("review") && !file.isEmpty()){
            ...
            if (!file.isEmpty()) {
                ...
                // 데이터 베이스 저장전에 잠시 저장
                com.example.dto.FileDto fileDto = new com.example.dto.FileDto();
    ...
    @ResponseBody
    public Map<String, Object> setMedUpdate(@ModelAttribute com.example.dto.MedicineDto medicineDto, @RequestParam("file") MultipartFile file) throws IOException {
    ...
    public String medDelete(@ModelAttribute com.example.dto.MedicineDto medicineDto){
        if( medicineDto.getMedId() > 0 && !medicineDto.getMedName().isEmpty() ){
            medicineService.deleteMed(medicineDto);
        }
        List<com.example.dto.FileDto> files = medicineService.getFiles(medicineDto.getMedId());
        for(com.example.dto.FileDto fd : files){
    ...
    public String boardView(@ModelAttribute com.example.dto.MedicineDto medicineDto , @RequestParam int medId, Model model){
